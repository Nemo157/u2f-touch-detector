...
impl Device {
    ...
    pub(crate) fn enumerate(hidapi: &hidapi::HidApi) -> impl Iterator<Item = Result<Self>> + '_ {
        hidapi
            .device_list()
            .filter(|info| info.usage_page() == FIDO_USAGE_PAGE && info.usage() == FIDO_USAGE_CTAPHID)
            .map(|info| Self::from_hidapi(hidapi, info))
    }

    #[culpa::try_fn]
    pub(crate) fn monitor(hidapi: &hidapi::HidApi) -> Result<impl Iterator<Item = Result<Self>> + '_> {
        hidapi
            .monitor()?
            .filter_map(|event| match event { hidapi::Event::Add(device) => Some(device), _ => None })
            .filter(|info| info.usage_page() == FIDO_USAGE_PAGE && info.usage() == FIDO_USAGE_CTAPHID)
            .map(|info| Self::from_hidapi(hidapi, &info))
    }
